...
app.post('/saveData', function(req, res) {
    ...
    request.query("Insert into register VALUES ('" + req.body.data.name + "', '" + req.body.data.city + "','" + img + "')");
...
function getCity(res, next, city) {
    var request = new mssql.Request();
    request.query(`Select * from register where city='${city}'`, function(err, result) {
        if (err) {
            return next(err);
        } else {
